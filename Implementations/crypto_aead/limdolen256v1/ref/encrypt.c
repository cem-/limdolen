...
// one round of Limdolen128
void e128Round(unsigned char *c, const unsigned char *k, const unsigned char roundConst) {
...
// this function performs all 16 rounds
void e256Rounds(unsigned char *c, const unsigned char *k) {
    //unsigned char temp;
...
//        unsigned char roundConst = getRoundConst(round_num);
        ...
        for(int i=0; i<CRYPTO_BYTES/2;i++) {
            //temp = c[i];
            c[i+CRYPTO_BYTES/2] ^= c[i];
            c[i] ^= c[i+CRYPTO_BYTES/2];
        }
    }
}
...
static void makeTag(const unsigned char *adKey, const unsigned char *ad, unsigned long long adlen,
    ...
    // these are only needed if the AAD and plaintext aren't multiples of crypto_bytes
